...
local ManCfg = { DelayKetik = 0.15, DelaySubmit = 0.45 }
...
local function TypeWord(word, delayKetik, delaySubmit)
    ...
    local startIndex = 1
    if #currentGUI > 0 and targetWord:sub(1, #currentGUI) == currentGUI then startIndex = #currentGUI + 1 end
    ...
    for i = 1, #suffix do
        ...
        if char:find("[aeiou]") then waitT = waitT * 0.7 end
...
local Win = Rayfield:CreateWindow({
    Name = "Maika Studio v2.9 Final",
    LoadingTitle = "Maika Studio Elite Engine v2.9",
...
local ToggleAuto = TBot:CreateToggle({
    ...
    Callback = function(v) 
        ...
        if v and ModeManual then
            ModeManual = false
            _G.ToggleManual:Set(false) -- Sync UI Position
            Rayfield:Notify({Title = "Mode Switch", Content = "Mode Manual dimatikan!"})
        end
    end
})
_G.ToggleAuto = ToggleAuto
...
TBot:CreateInput({
    ...
    Callback = function(v) AutoCfg.DelayTerima = tonumber(v) or 0.8 end
})

TBot:CreateInput({
    Name = "Speed Bot Ketikan",
    PlaceholderText = "Default: 0.15",
    Callback = function(v) AutoCfg.DelayKetik = tonumber(v) or 0.15 end
})

TBot:CreateInput({
    Name = "Speed Bot Submit",
    PlaceholderText = "Default: 0.45",
    Callback = function(v) AutoCfg.DelaySubmit = tonumber(v) or 0.45 end
})

local TMan = Win:CreateTab("Pilih Manual", nil)
local L_StatusMan = TMan:CreateLabel("Giliran: Menunggu...")

local ToggleManual = TMan:CreateToggle({
    ...
    Callback = function(v) 
        ...
        if v and ModeAuto then
            ModeAuto = false
            _G.ToggleAuto:Set(false) -- Sync UI Position
            Rayfield:Notify({Title = "Mode Switch", Content = "Mode Otomatis dimatikan!"})
        end
    end
})
_G.ToggleManual = ToggleManual
...
TMan:CreateInput({
    Name = "Speed Ketikan (Manual)",
    PlaceholderText = "0.15",
    Callback = function(v) ManCfg.DelayKetik = tonumber(v) or 0.15 end
})

TMan:CreateInput({
    Name = "Speed Submit (Manual)",
    PlaceholderText = "0.45",
    Callback = function(v) ManCfg.DelaySubmit = tonumber(v) or 0.45 end
})
...
local function RunBot(letter)
    ...
    CurThread = task.spawn(function()
        ...
        if chosen and TypeWord(chosen, AutoCfg.DelayKetik, AutoCfg.DelaySubmit) and IsMyTurn then
...
R_MatchUI.OnClientEvent:Connect(function(cmd, val)
    ...
    elseif cmd == "Mistake" then
        ...
        if ModeAuto and IsMyTurn then task.wait(0.2) RunBot(ServerLetter) end
    end
end)
...
R_Join:FireServer()
Rayfield:Notify({Title = "Maika Studio", Content = "V2.9: Hard Toggle Sync & All Features Ready!"})